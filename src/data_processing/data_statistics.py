...
def plot_median_rms(df, percentile_1=0.95, percentile_2=0.68, split=False, plot_mean=False, y_max=None, y_max2=None, x=[], save_path=None, title=''):
    mean = df.mean()
    median = df.median()
    ...
    if split:
        ...
        plt.subplot(211)
        plt.plot(x1, median1, label='median', linewidth=0.9)
        if plot_mean:
            plt.plot(x1, mean1, 'c', linestyle='--', label='mean')
        plt.fill_between(x1, quant05_1, quant95_1, color=c[col_num], alpha=0.8, label='5$^{th}$ and 95$^{th}$ percentile')
        #plt.fill_between(x1, quant32_1, quant68_1, color=(205 / 255, 210 / 255, 221 / 255), alpha=0.9,label=f'{percentile_2} percentile')
        if y_max == None:
        ...
        plt.subplot(212)
        plt.plot(x2, median2, label='median', linewidth=0.9)
        if plot_mean:
            plt.plot(x2, mean2, 'c', linestyle='--', label='mean')
        plt.fill_between(x2, quant05_2, quant95_2, color=c[col_num], alpha=0.8, label='5$^{th}$ and 95$^{th}$ percentile')
        #plt.fill_between(x2, quant32_2, quant68_2, color=(205 / 255, 210 / 255, 221 / 255), alpha=0.9,label=f'{percentile_2} percentile')
        if y_max2 == None:
...
def plot_mean_rms(df, split=False, y_max=None, y_max2=None, x=[]):
    mean = df.mean()
    std = df.std()
    if len(x) == 0:
